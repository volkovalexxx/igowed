  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(`/api/vendors/${vendorId}`, {
        ...
        body: JSON.stringify(mapProfileFormToPatch(form)),
      });
      if (!res.ok) {
  ...
  return (
    <div className={styles.page}>
      ...
      <div style={{ paddingBottom: 40 }}>
        {error && (
          <p role="alert" style={{ margin: '0 0 10px', color: '#e53131', fontSize: 13 }}>
            {error}
          </p>
        )}
        <button
          ...
          style={{ width: '100%', padding: '13px 24px', fontSize: 15, fontWeight: 600, borderRadius: 8, opacity: saving ? 0.7 : 1 }}
        >
          {saved
