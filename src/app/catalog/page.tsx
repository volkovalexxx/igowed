...
type SortOption = 'rating' | 'popular' | 'price_asc' | 'price_desc';
...
export default function CatalogPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (priceFrom) list = list.filter((v) => v.pricePerHour >= Number(priceFrom));
    if (priceTo)   list = list.filter((v) => v.pricePerHour <= Number(priceTo));
    ...
    switch (sortBy) {
      ...
      case 'price_asc':  list.sort((a, b) => a.pricePerHour - b.pricePerHour); break;
      case 'price_desc': list.sort((a, b) => b.pricePerHour - a.pricePerHour); break;
    }

    return list;
  }, [searchText, priceFrom, priceTo, ratingFilter, sortBy]);
