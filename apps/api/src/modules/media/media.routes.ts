...
export async function registerMediaRoutes(app: FastifyInstance, env: ApiEnv, storageOverride?: MediaStorage): Promise<void> {
  app.post('/api/v1/media/uploads', async (request, reply) => {
    try {
      const storage = storageOverride ?? createMediaStorage(env)
      const upload = await createPresignedUpload(request.body, storage)
